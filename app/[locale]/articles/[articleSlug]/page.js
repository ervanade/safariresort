...
const fetchArticle = async (slug, locale) => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_KEY}/api/v1/posts/${slug}?lang=${locale}`, {
...
export async function generateMetadata({ params }) {
  ...
  return getBaseMeta({
      ...
      title: 
          isEnglish ? article?.meta_title + " | Safari Resort Bogor" : article?.meta_title + " | Safari Resort Bogor" || article?.title + " | Safari Resort Bogor"
      ,
      description: 
          isEnglish ? article?.meta_desc : article?.meta_desc || ""
      ,
      keywords: 
