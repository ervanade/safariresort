...
const fetchRooms = async (locale, categorySlug) => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_KEY}/api/v1/accommodations?category=${categorySlug}&lang=${locale}`,
    {
...
const fetchCategory = async (locale, categorySlug) => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_KEY}/api/v1/categories/${categorySlug}?lang=${locale}`,
    {
      next: { revalidate: 60 * 5 },
      // cache: 'no-store',
      method: "GET",
...
const page = async ({ params }) => {
  ...
  const dataCategory = await fetchCategory(locale, categorySlug);
  const dataRooms = await fetchRooms(locale, categorySlug);
  //   const dataRooms = data.filter((room) =>
  //     room.name.toLowerCase().includes("treehouse"),
  //   );
  return (
    <div className="">
      <TreeHouse dataRooms={dataRooms} dataCategory={dataCategory || null} />
    </div>
  );
};
