...
export const mapFaqData = (apiData) => {
  ...
  return apiData.map((group) => {
    const config = categoryConfig[group.category] || {
      ...
      iconName: group.icon || "Info",
    };

    return {
      id: config.id,
      title: config.title,
      iconName: group.icon || config.iconName, // Mengirim string, bukan objek/fungsi
...
export const mapGatheringData = (apiData = []) => {
  return apiData.map((item) => {
    return {
      ...
      excerpt: item?.excerpt || "Reconnect with loved ones in the heart of nature. Perfect for large families looking for adventure and relaxation.",
      description:
        item?.description ||
        item?.excerpt ||
        "Reconnect with loved ones in the heart of nature. Perfect for large families looking for adventure and relaxation.",
      mainImage:
        item?.images[0] ||
