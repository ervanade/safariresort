...
import { 
  ...
  ConciergeBell, Clock, Bath, Luggage, Sun, Flower2, Wind,
  BedDouble, Coffee, ChevronLeft, ChevronRight
} from 'lucide-react';
...
const RoomDetail = ({roomSlug}) => {
  ...
  const amenities = room.amenities || room.features.slice(0, 8).map(f => ({ name: f, icon: 'ConciergeBell' }));
          ...
          <div className="max-w-xl mx-auto w-full pb-24 md:pb-0">
            ...
            <div className="mb-8">
              <div className="flex items-baseline gap-4 mb-4">
                <h1 className="text-4xl md:text-5xl font-bold text-[#7C3B1F]" style={{ fontFamily: 'Mikado, sans-serif' }}>
                  {room.name}
                </h1>
                <span className="text-xl text-gray-400 font-medium font-nunito">{room.size}</span>
              </div>
              ...
              <div className="flex gap-6 mb-10 pb-6 border-b border-[#7C3B1F]/10">
                 <div className="flex items-center gap-3 text-[#7C3B1F]/80">
                    <BedDouble size={24} />
                    <span className="text-sm font-bold max-w-[120px] leading-tight">1 Queen size bed</span>
                 </div>
                 <div className="flex items-center gap-3 text-[#7C3B1F]/80">
                    <Coffee size={24} />
                    <span className="text-sm font-bold max-w-[120px] leading-tight">Coffee & Tea included</span>
                 </div>
              </div>
            </div>
            ...
            {/* Amenities */}
            <div className="mb-10">
              <h3 className="text-xl font-bold text-[#7C3B1F] mb-6" style={{ fontFamily: 'Mikado, sans-serif' }}>Room Amenities</h3>
              <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                {amenities.map((item, idx) => {
                  const Icon = IconMap[item.icon] || ConciergeBell;
                  return (
                    <div key={idx} className="flex items-center gap-3 text-gray-600">
                      <Icon strokeWidth={1.5} className="w-6 h-6 text-[#7C3B1F]" />
                      <span className="text-sm font-medium font-nunito">{item.name}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {/* Bed Type */}
            <div className="mb-10">
               <h3 className="text-xl font-bold text-[#7C3B1F] mb-4" style={{ fontFamily: 'Mikado, sans-serif' }}>Bed Type</h3>
               <div className="flex items-start gap-4">
                  <BedDouble className="w-8 h-8 text-[#7C3B1F] mt-1" />
                  <p className="text-gray-600 text-sm font-medium max-w-xs">{bedType}</p>
               </div>
            </div>

            {/* Cancellation Rules */}
            <div className="mb-12 p-6 bg-[#faf7f5] border border-[#7C3B1F]/10 ">
               <h3 className="text-xl font-bold text-[#7C3B1F] mb-4" style={{ fontFamily: 'Mikado, sans-serif' }}>Cancellation Rules</h3>
               <p className="text-gray-500 text-sm font-medium mb-3">
                 Free Cancellation until <span className="text-[#7C3B1F] font-bold">22 October</span>
               </p>
               <p className="text-xs text-gray-400 leading-relaxed">
                 According to time at <strong className="text-gray-600">the accommodation</strong><br/>
                 <span className="text-[#F06934] font-bold">Untill 22 October 22:59 %100 Free</span><br/>
                 %100 money back
               </p>
            ...
            <div className="mt-auto border-t border-[#7C3B1F]/10 pt-8">
               <div className="flex flex-col gap-1 mb-4">
                 <div className="flex items-center gap-4">
                   <span className="text-gray-400 text-sm line-through decoration-gray-400">Normal Price</span>
                   <span className="text-gray-400 text-sm line-through">{room.originalPrice || 'IDR 3.500.000'}</span>
                   <span className="text-gray-400 text-sm">2 Nights</span>
                 </div>
                 <div className="flex items-center gap-4">
                   <span className="font-bold text-[#7C3B1F] text-sm w-20">Promo</span>
                   <span className="font-bold text-[#7C3B1F] text-lg">{room.promoPrice || room.price}</span>
                   <span className="text-[#7C3B1F] text-sm">2 Nights</span>
                 </div>
                 <div className="flex items-center gap-4 mt-2">
                   <span className="font-bold text-[#F06934] text-xl">Save Total</span>
                   <span className="font-bold text-[#F06934] text-xl">{room.savings || 'IDR 1.000.000'}</span>
                 </div>
               </div>
